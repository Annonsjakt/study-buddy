...
/**
 * The front page is for genuinely new visitors only: nothing of their own
 * yet, intro not seen, and not an installed-PWA launch (an app someone
 * added to their home screen should open the app, not a pitch for it).
 * Deep links are honoured elsewhere — this only ever governs the root route.
 */
 ...
 *  app's full navigation isn't the least distinguishable of several look-
 *  alike circular buttons. Closes on outside click, Esc, or navigation.
 *  `active` marks the tab itself current — the current page lives inside
 *  this menu rather than one of the bar's own three direct tabs. */
...
/** Fixed bottom tab bar — mobile's primary nav. Hidden on desktop (the
 *  sidebar already covers this ground there). Three direct destinations
 *  plus "Meny" for everything else, instead of every screen living behind
 *  one unlabeled icon among several look-alikes in the topbar. */
function tabBar() {
  const tabs = [
    { href: "#/", match: "/", icon: ICONS.home, label: t("nav.home") },
    { href: "#/study", match: "/study", icon: ICONS.clipboard, label: t("nav.study") },
...
async function render({ chromeOnly = false } = {}) {
  ...
  try {
    ...
    // bar around it. Only the landing page uses it: it brings its own header
