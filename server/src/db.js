...
db.exec(`
  ...
  -- above, there's no fixed direction. One row per pair, stored under a
  -- canonical ordering (user_a_id always the lexically smaller id), so it
  -- can be queried with a single OR clause instead of one row per direction.
  ...
  -- Same shape as invite_codes, kept separate rather than shared: a code
  -- means something different in each flow (redeeming creates a directional
  -- parent/student link there, a mutual friendship here), so mixing them in
  -- one table risked a code from one flow being redeemable in the other.
